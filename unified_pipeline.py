...
def build_pipeline_options(
    ...
    opts.do_formula_enrichment = True # Fast on GPU (~2-4s/image vs 35s on CPU)

    # ── Image Handling ───────────────────────────────────────────────────
    opts.generate_picture_images = True
    opts.generate_page_images = False
    opts.images_scale = 2.0 # Higher res for better VLM accuracy

    # ── Native VLM Picture Description (GPU-accelerated) ─────────────────
    # This reads diagrams, flowcharts, image-based content
    opts.do_picture_description = True
    opts.picture_description_options = PictureDescriptionVlmEngineOptions.from_preset(
        "smolvlm" # SmolVLM-256M — fast, fits easily in T4 VRAM alongside other models
    )
    # Override prompt with our custom extraction prompt
    opts.picture_description_options.prompt = PICTURE_DESCRIPTION_PROMPT

    # Filter small images (logos/icons) — skip images < 3% of page area
...
def inject_picture_descriptions(markdown: str, json_data: dict) -> str:
    ...
    while placeholder in remaining and desc_idx < len(descriptions):
        before, _, remaining = remaining.partition(placeholder)
